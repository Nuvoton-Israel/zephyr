...
LOG_MODULE_REGISTER(c2h_npcm, CONFIG_C2H_NPCM_LOG_LEVEL);
...
static void host_c2h_wait_write_done(const struct device *dev)
{
	...
	uint32_t max_wait_cycles = k_us_to_cyc_ceil32(NPCM_C2H_TRANSACTION_TIMEOUT_US);
...
static void __c2h_config_reg_access(const struct device *dev, SIB_DEVICE_T device, uint16_t offset)
{
	struct c2h_npcm_config *cfg = (struct c2h_npcm_config *)dev->config;
	struct c2h_reg *const inst_c2h = cfg->inst_c2h;
...
static void __c2h_write_reg(const struct device *dev, SIB_DEVICE_T device, uint16_t offset, uint8_t value)
{
	struct c2h_npcm_config *cfg = (struct c2h_npcm_config *)dev->config;
	struct c2h_reg *const inst_c2h = cfg->inst_c2h;
...
static uint8_t __c2h_read_reg(const struct device *dev, SIB_DEVICE_T device, uint16_t offset)
{
	struct c2h_npcm_config *cfg = (struct c2h_npcm_config *)dev->config;
	struct c2h_reg *const inst_c2h = cfg->inst_c2h;
	uint8_t data_val;
...
#define NPCM_C2H_INIT(inst)                                                                        \
	...
	static const struct c2h_npcm_config c2h_npcm_config_##inst = {                             \
		.inst_c2h = (struct c2h_reg *)DT_INST_REG_ADDR(inst),                              \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, c2h_init##inst, NULL, NULL, &c2h_npcm_config_##inst,           \
