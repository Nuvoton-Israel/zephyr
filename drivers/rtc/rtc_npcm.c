...
struct rtc_npcm_config {
	...
	const struct npcm4xx_wui rtcwk;
...
struct miwu_dev_callback rtc_miwu_cb;
...
static int rtc_npcm_init(const struct device *dev)
{
	...
	npcm4xx_miwu_init_dev_callback(&rtc_miwu_cb, &cfg->rtcwk, rtc_npcm_isr,
			dev);

	npcm4xx_miwu_manage_dev_callback(&rtc_miwu_cb, true);

	/*
	 * Configure the rtc wake-up event
	 */
	npcm4xx_miwu_interrupt_configure(&cfg->rtcwk,
			NPCM4XX_MIWU_MODE_EDGE, NPCM4XX_MIWU_TRIG_HIGH);

	npcm4xx_miwu_irq_enable(&cfg->rtcwk);
...
#define NPCM_RTC_INIT(inst)						                        \
	...
	static const struct rtc_npcm_config rtc_npcm_config_##inst = {	                        \
		...
		.rtcwk = NPCM4XX_DT_WUI_ITEM_BY_NAME(inst, rtc_wk),				\
