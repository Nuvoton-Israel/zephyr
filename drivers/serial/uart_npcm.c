...
#define DRV_CONFIG(dev) ((const struct uart_npcm_config *)(dev)->config)

#define DRV_DATA(dev) ((struct uart_npcm_data *)(dev)->data)

#define HAL_INSTANCE(dev) (struct uart_reg *)(DRV_CONFIG(dev)->base)
...
static void uart_npcm_irq_callback_set(const struct device *dev,
					 ...
					 void *cb_data)
{
	struct uart_npcm_data *data = DRV_DATA(dev);
...
void uart_set_npcm_baud_rate(const struct device *dev, int baud_rate, uint32_t src_clk)
{
	...
	struct uart_npcm_data *const data = DRV_DATA(dev);
	struct uart_reg *const inst = HAL_INSTANCE(dev);
...
{
	const struct uart_npcm_config *const config = DRV_CONFIG(dev);
	struct uart_npcm_data *const data = DRV_DATA(dev);
	struct uart_reg *const inst = HAL_INSTANCE(dev);
	...
	uart_set_npcm_baud_rate(dev, data->ucfg.baudrate, uart_rate);
...
{
	const struct uart_npcm_data *const data = DRV_DATA(dev);
