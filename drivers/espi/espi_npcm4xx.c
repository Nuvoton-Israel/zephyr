 ...
 * npcm4xx-espi-vws-map.dtsi device tree file for more detail.
 */
static const struct npcm4xx_vw_in_config vw_in_tbl[] = {
	/* index 02h (In)  */
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SLP_S3, vw_slp_s3),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SLP_S4, vw_slp_s4),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SLP_S5, vw_slp_s5),
	/* index 03h (In)  */
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SUS_STAT, vw_sus_stat),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_PLTRST, vw_plt_rst),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_OOB_RST_WARN, vw_oob_rst_warn),
	/* index 07h (In)  */
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_HOST_RST_WARN, vw_host_rst_warn),
	/* index 41h (In)  */
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SUS_WARN, vw_sus_warn),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SUS_PWRDN_ACK, vw_sus_pwrdn_ack),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SLP_A, vw_slp_a),
	/* index 42h (In)  */
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SLP_LAN, vw_slp_lan),
	NPCM4XX_DT_VW_IN_CONF(ESPI_VWIRE_SIGNAL_SLP_WLAN, vw_slp_wlan),
};

static const struct npcm4xx_vw_out_config vw_out_tbl[] = {
	/* index 04h (Out) */
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_OOB_RST_ACK, vw_oob_rst_ack),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_WAKE, vw_wake),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_PME, vw_pme),
	/* index 05h (Out) */
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_SLV_BOOT_DONE, vw_slv_boot_done),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_ERR_FATAL, vw_err_fatal),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_ERR_NON_FATAL, vw_err_non_fatal),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_SLV_BOOT_STS,
						vw_slv_boot_sts_with_done),
	/* index 06h (Out) */
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_SCI, vw_sci),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_SMI, vw_smi),
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_HOST_RST_ACK, vw_host_rst_ack),
	/* index 40h (Out) */
	NPCM4XX_DT_VW_OUT_CONF(ESPI_VWIRE_SIGNAL_SUS_ACK, vw_sus_ack),
};
...
static int espi_npcm4xx_receive_vwire(const struct device *dev,
				enum espi_vwire_signal signal, uint8_t *level);
static int espi_npcm4xx_send_vwire(const struct device *dev,
			enum espi_vwire_signal signal, uint8_t level);
		...
		miwu_dev_callback_handler_t handler)
{
	...
	if (wui->table == NPCM4XX_MIWU_TABLE_NONE)
		return;

	/* Install callback function */
	npcm4xx_miwu_init_dev_callback(callback, wui, handler, dev);
	npcm4xx_miwu_manage_dev_callback(callback, 1);

	/* Congiure MIWU setting and enable its interrupt */
	npcm4xx_miwu_interrupt_configure(wui, NPCM4XX_MIWU_MODE_EDGE,
							NPCM4XX_MIWU_TRIG_BOTH);
	npcm4xx_miwu_irq_enable(wui);
}
...
static void espi_bus_cfg_update_isr(const struct device *dev)
{
	...
	if ((chg_mask & BIT(NPCM4XX_ESPI_CH_VW)) && IS_BIT_SET(inst->ESPICFG,
...
static void espi_bus_generic_isr(void *arg)
{
	 ...
	 */
	mask = inst->ESPIIE | (1 << NPCM4XX_ESPISTS_VWUPDW);
...
static void espi_vw_config_input(const struct device *dev,
				const struct npcm4xx_vw_in_config *config_in)
{
	struct espi_reg *const inst = HAL_INSTANCE(dev);
	int idx = config_in->reg_idx;

	/* IE & WE bits are already set? */
	if (IS_BIT_SET(inst->VWEVMS[idx], NPCM4XX_VWEVMS_IE) &&
		IS_BIT_SET(inst->VWEVMS[idx], NPCM4XX_VWEVMS_WE))
		return;

	/* Set IE & WE bits in VWEVMS */
	inst->VWEVMS[idx] |= BIT(NPCM4XX_VWEVMS_IE) | BIT(NPCM4XX_VWEVMS_WE);
	LOG_DBG("VWEVMS%d 0x%08X", idx, inst->VWEVMS[idx]);
}

static void espi_vw_config_output(const struct device *dev,
				const struct npcm4xx_vw_out_config *config_out)
{
	struct espi_reg *const inst = HAL_INSTANCE(dev);
	int idx = config_out->reg_idx;
	uint8_t valid = GET_FIELD(inst->VWEVSM[idx], NPCM4XX_VWEVSM_VALID);

	/* Set valid bits for vw signal which we have declared in table. */
	valid |= config_out->bitmask;
	SET_FIELD(inst->VWEVSM[idx], NPCM4XX_VWEVSM_VALID, valid);
	 ...
	 */
	SET_FIELD(inst->VWEVSM[idx], NPCM4XX_VWEVSM_HW_WIRE, 0);
				...
				enum espi_vwire_signal signal)
{
	...
	k_busy_wait(NPCM4XX_ESPI_VWIRE_ACK_DELAY);
	switch (signal) {
	case ESPI_VWIRE_SIGNAL_HOST_RST_WARN:
		espi_npcm4xx_send_vwire(dev,
				ESPI_VWIRE_SIGNAL_HOST_RST_ACK,
				wire);
		break;
	case ESPI_VWIRE_SIGNAL_SUS_WARN:
		espi_npcm4xx_send_vwire(dev, ESPI_VWIRE_SIGNAL_SUS_ACK,
				wire);
		break;
	case ESPI_VWIRE_SIGNAL_OOB_RST_WARN:
		espi_npcm4xx_send_vwire(dev, ESPI_VWIRE_SIGNAL_OOB_RST_ACK,
				wire);
		break;
	default:
		break;
...
static void espi_vw_send_bootload_done(const struct device *dev)
{
	...
	ret = espi_npcm4xx_receive_vwire(dev,
	...
	if (!ret && !boot_done) {
		...
		espi_npcm4xx_send_vwire(dev, ESPI_VWIRE_SIGNAL_SLV_BOOT_STS, 1);
	}
}

static void espi_vw_generic_isr(const struct device *dev, struct npcm4xx_wui *wui)
{
	int idx;
	enum espi_vwire_signal signal;
...
static void espi_vw_espi_rst_isr(const struct device *dev, struct npcm4xx_wui *wui)
{
	struct espi_reg *const inst = HAL_INSTANCE(dev);
	struct espi_npcm4xx_data *const data = DRV_DATA(dev);
	...
	data->espi_rst_asserted = IS_BIT_SET(inst->ESPISTS,
						NPCM4XX_ESPISTS_ESPIRST_DEASSERT);
			...
			enum espi_vwire_signal signal, uint8_t level)
{
	...
	val = GET_FIELD(inst->VWEVSM[reg_idx], NPCM4XX_VWEVSM_WIRE);
	if (level)
		val |= bitmask;
	else
		val &= ~bitmask;

	SET_FIELD(inst->VWEVSM[reg_idx], NPCM4XX_VWEVSM_WIRE, val);
...
static int espi_npcm4xx_read_lpc_request(const struct device *dev,
...
#if defined(CONFIG_ESPI_OOB_CHANNEL)
static int espi_npcm4xx_send_oob(const struct device *dev,
				struct espi_oob_packet *pckt)
{
	...
	if (sz_oob_tx > NPCM4XX_ESPI_OOB_MAX_PAYLOAD) {
		LOG_ERR("Out of OOB transmitted buffer: %d", sz_oob_tx);
		return -EINVAL;
	}

	/* Check OOB Transmit Queue is empty? */
	if (IS_BIT_SET(inst->OOBCTL, NPCM4XX_OOBCTL_OOB_AVAIL)) {
		LOG_ERR("OOB channel is busy");
		return -EBUSY;
	}

	/*
	 * GET_OOB header (first 4 bytes) in npcm4xx 32-bits tx buffer
	 *
	 ...
	 * [0:7]   - SZ_PACK      Package size plus 3 bytes header. (Npcm4xx only)
	 */
	 ...
	 */
	oob_data = inst->OOBCTL & ~(BIT(NPCM4XX_OOBCTL_OOB_FREE));
	oob_data |= BIT(NPCM4XX_OOBCTL_OOB_AVAIL);
	inst->OOBCTL = oob_data;

	while (IS_BIT_SET(inst->OOBCTL, NPCM4XX_OOBCTL_OOB_AVAIL))
		;
...
static int espi_npcm4xx_receive_oob(const struct device *dev,
				struct espi_oob_packet *pckt)
{
	struct espi_reg *const inst = HAL_INSTANCE(dev);
	struct espi_npcm4xx_data *const data = DRV_DATA(dev);
	...
	/*
	 * PUT_OOB header (first 4 bytes) in npcm4xx 32-bits rx buffer
	 *
	 ...
	 * [0:7]   - SZ_PACK      Reserved. (Npcm4xx only)
	 */
	oob_data = inst->OOBRXBUF[0];
	/* Get received package length first */
	sz_oob_rx = NPCM4XX_OOB_RX_PACKAGE_LEN(oob_data);

	/* Check OOB received buffer size */
	if (sz_oob_rx > NPCM4XX_ESPI_OOB_MAX_PAYLOAD) {
...
void npcm4xx_espi_enable_interrupts(const struct device *dev)
{
	ARG_UNUSED(dev);
	...
	for (int idx = 0; idx < ARRAY_SIZE(vw_in_tbl); idx++) {
		npcm4xx_miwu_irq_enable(&(vw_in_tbl[idx].vw_wui));
	}
}

void npcm4xx_espi_disable_interrupts(const struct device *dev)
{
	ARG_UNUSED(dev);
	...
	for (int idx = 0; idx < ARRAY_SIZE(vw_in_tbl); idx++) {
		npcm4xx_miwu_irq_disable(&(vw_in_tbl[idx].vw_wui));
	}
}
...
static const struct espi_driver_api espi_npcm4xx_driver_api = {
	...
	.send_vwire = espi_npcm4xx_send_vwire,
	.receive_vwire = espi_npcm4xx_receive_vwire,
	.manage_callback = espi_npcm4xx_manage_callback,
	.read_lpc_request = espi_npcm4xx_read_lpc_request,
	.write_lpc_request = espi_npcm4xx_write_lpc_request,
#if defined(CONFIG_ESPI_OOB_CHANNEL)
	.send_oob = espi_npcm4xx_send_oob,
	.receive_oob = espi_npcm4xx_receive_oob,
#endif
};
