...
#define DT_DRV_COMPAT nuvoton_npcx_spim_nor
...
LOG_MODULE_REGISTER(flash_npcx_spim_nor, CONFIG_FLASH_LOG_LEVEL);
...
static int flash_npcx_transceive(const struct device *dev, struct npcx_transceive_cfg *cfg,
				     uint32_t flags)
{
	const struct flash_npcx_nor_config *config = dev->config;
	struct flash_npcx_nor_data *data = dev->data;
	struct npcx_qspi_data *qspi_data = config->qspi_bus->data;
	int ret;
...
/* NPCX functions for SPI NOR flash */
static int flash_npcx_transceive_cmd_only(const struct device *dev, uint8_t opcode)
{
	struct npcx_transceive_cfg cfg = { .opcode = opcode};

	return flash_npcx_transceive(dev, &cfg, 0); /* opcode only */
}

static int flash_npcx_transceive_cmd_by_addr(const struct device *dev, uint8_t opcode,
					     uint32_t addr)
{
	struct npcx_transceive_cfg cfg = { .opcode = opcode};

	cfg.addr.u32 = sys_cpu_to_be32(addr);
	return flash_npcx_transceive(dev, &cfg, NPCX_TRANSCEIVE_ACCESS_ADDR);
}

static int flash_npcx_transceive_read_by_addr(const struct device *dev, uint8_t opcode,
					      uint8_t *dst, const size_t size, uint32_t addr)
{
	...
	cfg.addr.u32 = sys_cpu_to_be32(addr);
	return flash_npcx_transceive(dev, &cfg, NPCX_TRANSCEIVE_ACCESS_READ |
					NPCX_TRANSCEIVE_ACCESS_ADDR);
}

static int flash_npcx_transceive_read(const struct device *dev, uint8_t opcode,
...
static int flash_npcx_transceive_write(const struct device *dev, uint8_t opcode,
				       uint8_t *src, const size_t size)
{
	...
	return flash_npcx_transceive(dev, &cfg, NPCX_TRANSCEIVE_ACCESS_WRITE);
}

static int flash_npcx_transceive_write_by_addr(const struct device *dev, uint8_t opcode,
...
static int flash_npcx_nor_wait_until_ready(const struct device *dev)
{
	...
	do {
		ret = flash_npcx_transceive_read(dev, SPI_NOR_CMD_RDSR, &reg, sizeof(reg));
		if (ret != 0) {
			return ret;
...
{
	int ret = flash_npcx_transceive_read(dev, SPI_NOR_CMD_RDSR, sts_reg, 1);

	if (ret != 0) {
		return ret;
	}
	return flash_npcx_transceive_read(dev, SPI_NOR_CMD_RDSR2, sts_reg + 1, 1);
}
...
static int flash_npcx_nor_write_status_regs(const struct device *dev, uint8_t *sts_reg)
{
	...
	ret = flash_npcx_transceive_cmd_only(dev, SPI_NOR_CMD_WREN);
	if (ret != 0) {
		return ret;
	}

	ret = flash_npcx_transceive_write(dev, SPI_NOR_CMD_WRSR, sts_reg, 2);
	if (ret != 0) {
		return ret;
	}
...
static int flash_npcx_nor_read_jedec_id(const struct device *dev, uint8_t *id)
{
	...
	return flash_npcx_transceive_read(dev, SPI_NOR_CMD_RDID, id, SPI_NOR_MAX_ID_LEN);
}
...
{
	uint8_t sfdp_addr[4];
	struct npcx_transceive_cfg cfg = { .opcode = JESD216_CMD_READ_SFDP,
					.tx_buf = sfdp_addr,
					.tx_count = 4,
					.rx_buf = data,
					.rx_count = size};
				 ...
				 void *data, size_t size)
{
	...
	if (config->mapped_addr == MAPPED_ADDR_NOT_SUPPORT) {
		return flash_npcx_transceive_read_by_addr(dev,
...
static int flash_npcx_nor_erase(const struct device *dev, off_t addr, size_t size)
{
	...
	if (size == config->flash_size) {
		flash_npcx_transceive_cmd_only(dev, SPI_NOR_CMD_WREN);
		/* Send chip erase command */
		flash_npcx_transceive_cmd_only(dev, SPI_NOR_CMD_CE);
		return flash_npcx_nor_wait_until_ready(dev);
	}
	...
	while (size > 0) {
		...
		if ((size >= BLOCK_64K_SIZE) && SPI_NOR_IS_64K_ALIGNED(addr)) {
			flash_npcx_transceive_cmd_by_addr(dev, SPI_NOR_CMD_BE, addr);
			addr += BLOCK_64K_SIZE;
			size -= BLOCK_64K_SIZE;
		} else {
			flash_npcx_transceive_cmd_by_addr(dev, SPI_NOR_CMD_SE, addr);
			addr += BLOCK_4K_SIZE;
			size -= BLOCK_4K_SIZE;
		}
				  ...
				  const void *data, size_t size)
{
	...
	while (size > 0) {
		/* Start to write */
		flash_npcx_transceive_cmd_only(dev, SPI_NOR_CMD_WREN);
		ret = flash_npcx_transceive_write_by_addr(dev, SPI_NOR_CMD_PP, tx_buf,
						   sz_write, addr);
		if (ret != 0) {
			break;
		}
...
static int flash_npcx_nor_ex_exec_transceive(const struct device *dev,
				      const struct npcx_ex_ops_transceive_in *op_in,
				      const struct npcx_ex_ops_transceive_out *op_out)
{
	int flag = 0;
	struct npcx_transceive_cfg cfg;
				...
				const uintptr_t in, void *out)
{
	...
	switch (code) {
	case FLASH_NPCX_EX_OP_EXEC_TRANSCEIVE:
	{
		struct npcx_ex_ops_transceive_in *op_in =
			(struct npcx_ex_ops_transceive_in *)in;
		struct npcx_ex_ops_transceive_out *op_out =
			(struct npcx_ex_ops_transceive_out *)out;
#ifdef CONFIG_USERSPACE
		struct npcx_ex_ops_transceive_in in_copy;
		struct npcx_ex_ops_transceive_out out_copy;
		...
		ret = flash_npcx_nor_ex_exec_transceive(dev, op_in, op_out);
#ifdef CONFIG_USERSPACE
...
static int flash_npcx_nor_init(const struct device *dev)
{
	...
	if (config->qspi_cfg.enter_4ba != 0) {
		...
		ret = flash_npcx_transceive_cmd_only(dev, SPI_NOR_CMD_4BA);
		if (ret != 0) {
