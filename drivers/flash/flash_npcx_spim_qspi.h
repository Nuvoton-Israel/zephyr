...
#ifndef ZEPHYR_DRIVERS_FLASH_NPCX_QSPI_H_
#define ZEPHYR_DRIVERS_FLASH_NPCX_QSPI_H_
...
#define NPCX_TRANSCEIVE_ACCESS_WRITE BIT(0)
#define NPCX_TRANSCEIVE_ACCESS_READ  BIT(1)
#define NPCX_TRANSCEIVE_ACCESS_ADDR  BIT(2)
...
#define NPCX_EX_OP_LOCK_TRANSCEIVE	BIT(0) /* Lock/Unlock transceive */
#define NPCX_EX_OP_INT_FLASH_WP 	BIT(1) /* Issue write protection of internal flash */
#define NPCX_EX_OP_EXT_FLASH_WP 	BIT(2) /* Issue write protection of external flash */

/* Transceive operation configuration for a SPI device */
struct npcx_transceive_cfg {
	uint8_t opcode;
	uint8_t *tx_buf;
	size_t  tx_count;
	uint8_t *rx_buf;
 ...
 */
typedef int (*qspi_npcx_transceive)(const struct device *dev,
 ...
 */
typedef void (*qspi_npcx_mutex_lock_configure)(const struct device *dev,
 ...
 * @param dev Pointer to the device structure for qspi bus controller instance.
 */
typedef void (*qspi_npcx_mutex_unlock)(const struct device *dev);

struct npcx_qspi_ops {
	qspi_npcx_mutex_lock_configure lock_configure;
	qspi_npcx_mutex_unlock unlock;
	qspi_npcx_transceive transceive;
};

/* Device data */
struct npcx_qspi_data {
	/* mutex of qspi bus controller */
	struct k_sem lock_sem;
	/* qspi operation interface */
	struct npcx_qspi_ops *qspi_ops;
...
#endif /* ZEPHYR_DRIVERS_FLASH_NPCX_QSPI_H_ */
