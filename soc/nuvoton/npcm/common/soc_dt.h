...
#ifndef _NUVOTON_NPCM_SOC_DT_H_
#define _NUVOTON_NPCM_SOC_DT_H_
 ...
 */
#define NPCX_DT_CLK_CFG_ITEMS_FUNC(child, inst) NPCX_DT_CLK_CFG_ITEM_BY_IDX(inst, child)
 ...
 */
#define NPCX_DT_PHANDLE_FROM_WUI_NAME(inst, name) DT_INST_PHANDLE(inst, name)
...
#define NPCX_DT_WUI_ITEM_BY_NAME(inst, name)                                                       \
	{                                                                                          \
		.table = DT_PROP(DT_PHANDLE(NPCX_DT_PHANDLE_FROM_WUI_NAME(inst, name), miwus),     \
				 index),                                                           \
		.group = DT_PHA(NPCX_DT_PHANDLE_FROM_WUI_NAME(inst, name), miwus, group),          \
		.bit = DT_PHA(NPCX_DT_PHANDLE_FROM_WUI_NAME(inst, name), miwus, bit),              \
	}
 ...
 */
#define NPCX_DT_PHANDLE_FROM_WUI_MAPS(inst, i) DT_INST_PHANDLE_BY_IDX(inst, wui_maps, i)
...
#define NPCX_DT_WUI_ITEM_BY_IDX(inst, i)                                                           \
	{                                                                                          \
		.table =                                                                           \
			DT_PROP(DT_PHANDLE(NPCX_DT_PHANDLE_FROM_WUI_MAPS(inst, i), miwus), index), \
		.group = DT_PHA(NPCX_DT_PHANDLE_FROM_WUI_MAPS(inst, i), miwus, group),             \
 ...
 */
#define NPCX_DT_NODE_FROM_MIWU_MAP(i)     DT_PATH(npcx_miwus_int_map, map_miwu##i##_groups)
/**
 ...
 * "nuvoton,npcx-miwu".
 */
#define NPCX_DT_MIWU_IRQ_TABLE_IDX(child) DT_PROP(DT_PHANDLE(DT_PARENT(child), parent), index)
 ...
 */
#define NPCX_DT_NODE_FROM_VWTABLE(name) DT_CHILD(DT_PATH(npcx_espi_vws_map), name)
 ...
 */
#define NPCX_DT_PHANDLE_VW_WUI(name) DT_PHANDLE(NPCX_DT_NODE_FROM_VWTABLE(name), vw_wui)
...
#define NPCX_DT_VW_WUI_ITEM(name)                                                                  \
	{                                                                                          \
		.table = DT_PROP(DT_PHANDLE(NPCX_DT_PHANDLE_VW_WUI(name), miwus), index),          \
...
#define NPCX_DT_VW_IN_CONF(signal, name)                                                           \
	{                                                                                          \
		.sig = signal,                                                                     \
		.reg_idx = DT_PROP_BY_IDX(NPCX_DT_NODE_FROM_VWTABLE(name), vw_reg, 0),             \
		.bitmask = DT_PROP_BY_IDX(NPCX_DT_NODE_FROM_VWTABLE(name), vw_reg, 1),             \
 ...
 */
#define NPCX_DT_LVOL_CTRL_NONE DT_PHA(DT_NODELABEL(lvol_none), lvols, ctrl)
...
#define NPCX_DT_LVOL_ITEMS_LIST(inst)                                                              \
	{DT_FOREACH_PROP_ELEM(DT_DRV_INST(inst), lvol_maps, NPCX_DT_LVOL_ITEMS_INIT)}
 ...
 *         FALSE - otherwise.
 */
#define NPCX_BOOTER_IS_HIF_TYPE_SET() DT_PROP(DT_PATH(booter_variant), hif_type_auto)
