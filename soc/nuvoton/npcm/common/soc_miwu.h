...
BUILD_ASSERT(NPCX_MIWU_TABLE_COUNT == NPCM4_MIWU_TABLE_COUNT,
	     "MIWU table count does not match the npcm4 binding");
BUILD_ASSERT(NPCX_MIWU_GROUP_COUNT == NPCM4_MIWU_GROUP_COUNT,
	     "MIWU group count does not match the npcm4 binding");
 ...
 */
typedef void (*miwu_dev_callback_handler_t)(const struct device *source, struct npcx_wui *wui);
 ...
 */
int npcx_miwu_interrupt_configure(const struct npcx_wui *wui, enum miwu_int_mode mode,
 ...
 */
void npcx_miwu_init_gpio_callback(struct miwu_callback *callback, const struct npcx_wui *io_wui,
				  int port);
 ...
 */
void npcx_miwu_init_dev_callback(struct miwu_callback *callback, const struct npcx_wui *dev_wui,
