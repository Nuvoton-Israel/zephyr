...
def GenFWHeader():
    ...
    try:
        ...
        # hDevMode
        temp = [int(Dict_FW_H['hNotDoBackup'], 0), int(Dict_FW_H['hOTPRefToSrcTable'], 0), int(Dict_FW_H['hNotEraseOTPTable'], 0), int(Dict_FW_H['hNotUpdateOTPRegister'], 0), int(Dict_FW_H['hHwTrimRefOTPTable'], 0), int(Dict_FW_H['hOTPRefToTable'], 0), int(Dict_FW_H['hSecurityLvl'], 0),
                int(Dict_FW_H['hSecureBoot'], 0)]
        ListFWHeaderCol_Sign.append(Fn.ConvertBitArray2Byte(temp))

        # hFlashLockReg0
        ListFWHeaderCol_Sign.append(
            (int(Dict_FW_H['hFlashLockReg0'], 0).to_bytes(1, byteorder='big')))
        ...
        # AES Encryption
        aes_type = int(Dict_OTP['oAESDecryptEn'], 10)
        if(AESencrypt):
        ...
        if(Signature):
            if(int(Dict_Config["CryptoSelect"]) == 0):
                ...
                ListFWHeaderCol.append(OpenSSL_SignFile(HashTypeforECKey, (int)(Dict_OTP['oRSAPKCPAD']), Fn.GenBinFilefromList(
                    hHashTotal, 'SignField'), Dict_OpenSSL['EcFwSigKey']))
            elif(int(Dict_Config["CryptoSelect"]) == 1):
                if((int)(Dict_CNG['SignCert_Idx']) == 0):
                    ListFWHeaderCol.append(CNG_SignFile(
                        Dict_CNG["EcFwCert0_Subject"], (int)(Dict_OTP['oRSAPKCPAD']), HashTypeforECKey, 0, Fn.GenBinFilefromList(hHashTotal, 'SignField')))
                elif((int)(Dict_CNG['SignCert_Idx']) == 1):
                    ListFWHeaderCol.append(CNG_SignFile(
                        Dict_CNG["EcFwCert1_Subject"], (int)(Dict_OTP['oRSAPKCPAD']), HashTypeforECKey, 0, Fn.GenBinFilefromList(hHashTotal, 'SignField')))
            else:
            ...
            ListFWHeaderCol.append(Fn.ReservedData(
                (512 - (HashTypeforECKey/8)) * 8))
...
def GenHeader():
    ...
    # ActiveEcFwOffset
    mcp_size = int(Dict_OTP['oMCPFlashSize'], 10)
    if(mcp_size == 0):
        # auto mode, default 8K Bytes (2M/256)
        ActiveEcFwOffset = int(Dict_FW_H['hActiveEcFwOffset'], 0) * 8192
    else:
        ActiveEcFwOffset = int(
    ...
    if Dict_FW_H['hUserFWRamCodeFlashStart'] is not None:
        ...
        RAMCodeStart = Fn.Bytes2Int(
            hUserFWRamCodeFlashStart[0:4], 'little') - ActiveEcFwOffset
    ...
    if Dict_FW_H['hUserFWRamCodeFlashEnd'] is not None:
        ...
        RAMCodeEnd = Fn.Bytes2Int(
            hUserFWRamCodeFlashEnd[0:4], 'little') - ActiveEcFwOffset
