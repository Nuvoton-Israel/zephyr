...
static void test_i3c_slave_task(void *arg1, void *arg2, void *arg3)
{
	...
	for (;;) {
		...
		while (1) {
			...
			if (ret) {
				if (memcmp(test_data_tx_mst, test_data_rx_slv, TEST_PRIV_XFER_SIZE) == 0) {
					break;
				} else {
					LOG_WRN("$");
				}
		...
		ast_zassert_mem_equal(test_data_tx_mst, test_data_rx_slv, TEST_PRIV_XFER_SIZE, "i3c private write test fail: data mismatch %d %X %X", counter, test_data_tx_mst[0], test_data_rx_slv[0]);
...
static void test_i3c_ci(int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (IS_MDB_PENDING_READ_NOTIFY(mdb)) {
			...
			/* check result */
			ast_zassert_equal(mdb, test_data_rx_mst[0], "IBI MDB mismatch: %02x %02x\n", ret, test_data_rx_mst[0]);
		}
